...
[ -f "requirements.txt" ] && echo "🐍 Backend: Python" && head -3 requirements.txt 2>/dev/null | sed 's/^/   /'
...
[ -f "README.md" ] && echo "📖 README available"

# Biggest directory
echo ""
echo "📁 MAIN CODE:"
ls -la | grep -E "^d.* (src|backend|app|lib|public)$" | head -5 | awk '{print "  " $NF}'

# Last modified
echo ""
echo "🕐 RECENT CHANGES:"
find . -name "*.ts" -o -name "*.tsx" -o -name "*.py" -o -name "*.js" 2>/dev/null | grep -v node_modules | grep -v __pycache__ | xargs ls -lt 2>/dev/null | head -3 | awk '{print "  " $6 " " $7 " " $9}'
